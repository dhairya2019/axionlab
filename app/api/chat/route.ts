...
export async function POST(request: Request) {
  ...
  try {
    ...
    const functionCall = response.candidates?.[0]?.content?.parts?.find((p: { functionCall?: unknown }) => p.functionCall);

    if (functionCall && (functionCall as { functionCall: { name: string; args: Record<string, string> } }).functionCall.name === 'sendEmailInquiry') {
      const args = (functionCall as { functionCall: { name: string; args: Record<string, string> } }).functionCall.args;
