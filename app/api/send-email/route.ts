...
export async function POST(request: Request) {
  ...
  if (!process.env.SENDGRID_API_KEY) {
    ...
    return Response.json({ error: 'Mail server is currently unconfigured.' }, { status: 500 });
  }
  ...
  try {
    ...
    console.error('SendGrid Error:', (error as { response?: { body: unknown } }).response?.body || error);
    return Response.json({ error: 'Failed to dispatch email. Please try again later.' }, { status: 500 });
  }
}
